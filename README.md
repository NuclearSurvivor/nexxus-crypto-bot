...
- **`install_windows.bat`** — double-click installer: creates a `.venv`, installs all dependencies, converts `icon.png → icon.ico`, writes `run_windows.bat`, and pins Desktop + Start Menu shortcuts.
- **`install_windows.ps1`** — the PowerShell script behind the installer. No administrator rights required. Checks Python 3.9+, gives a clear download link if missing.
- **`run_windows.bat`** — launches via `.venv\Scripts\pythonw.exe` (no console window) with fallback to system Python.
- **Windows asyncio policy** — `WindowsSelectorEventLoopPolicy` set on startup to avoid `ProactorEventLoop` edge-case issues with websockets on Windows 10/11.
- **Windows refresh rate** — `_detect_monitor_hz()` now queries `GetDeviceCaps(VREFRESH)` via ctypes on Windows instead of calling the Linux-only `xrandr` binary. Accurate Hz throttling on any Windows display.
- **Cross-platform file paths** — all path handling already used `os.path.join`; confirmed no hard-coded POSIX separators.
...
1. Install **Python 3.9+** from [python.org/downloads](https://www.python.org/downloads/windows/)
   - During setup, tick **"Add Python to PATH"**
2. Download or clone this repo to any folder (e.g. `C:\Users\YourName\NEXXUS`)
3. Double-click **`install_windows.bat`**

The installer will:
- Create a `.venv` virtual environment inside the project folder
- Install all Python dependencies into it
- Convert `icon.png` → `icon.ico`
- Add a **Desktop shortcut** and a **Start Menu** entry pointing at `pythonw.exe` (no console window)
- Write `run_windows.bat` for manual launches

After it finishes, double-click **NEXXUS Crypto Bot** on your Desktop to launch.
...
python -m venv .venv
.venv\Scripts\pip install customtkinter pillow coinbase-advanced-py matplotlib websockets numpy pytz
...
├── crash.log               # Crash output captured by Python exception handler
├── icon.png                # App icon (source)
├── icon.ico                # App icon for Windows (auto-generated by installer)
│
├── install.sh              # Linux installer — deps + desktop shortcut
├── run.sh                  # Linux launcher (uses NEXXUS pyenv venv)
│
├── install_windows.bat     # Windows installer — double-click to run
├── install_windows.ps1     # Windows PowerShell installer (called by .bat)
└── run_windows.bat         # Windows launcher — double-click to start the bot
```
